...
//перевірка щоб дата не припадала на неділю
  ...
  endDate.addEventListener("input", () => (lastChanged = "end"));
  ...
  const loadProfile = (of) => {
    ...
    try {
      ...
      if (data.holidaysText != null) {
        holidaysArea.value = data.holidaysText;
      } else {
        holidaysArea.value = "";
      }
    } catch {
...
// відображення залишку на екрані
  ...
  const calculate = () => {
    ...
    if (!lastChanged) lastChanged = "duration";

    let warnings = [];

    if (s && d !== null && (lastChanged === "start" || lastChanged === "duration")) {
